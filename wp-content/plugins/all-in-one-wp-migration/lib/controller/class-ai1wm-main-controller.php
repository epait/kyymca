...
class Ai1wm_Main_Controller {
	...
	private function activate_actions() {
		...
		add_action( 'init', array( $this, 'admin_ajax' ) );
		add_action( 'init', array( $this, 'create_folders' ) );
		add_action( 'init', array( $this, 'http_authentication' ) );
	...
	public function multisite_notice() {
		...
		<div class="error">
			<p>
				...
				?>
				<a href="https://servmask.com/extensions/multisite" target="_blank" class="ai1wm-label">
	...
	public function create_folders() {
		...
		if ( ! file_exists( AI1WM_STORAGE_PATH ) ) {
			...
			if ( ! mkdir( AI1WM_STORAGE_PATH ) ) {
				// We couldn't create the folder, no point to continue. let's tell the user
				wp_die(
					__(
						'We were unable to create ' . AI1WM_STORAGE_PATH . ' folder. ' .
		...
		if ( ! file_exists( AI1WM_BACKUPS_PATH ) ) {
			...
			if ( ! mkdir( AI1WM_BACKUPS_PATH ) ) {
				...
				wp_die(
					__(
						'We were unable to create ' . AI1WM_BACKUPS_PATH . ' folder. ' .
						'You will need to create this folder and grant it read/write/execute permissions (0777) ' .
						'for the All in One WP Migration plugin to function properly.',
						AI1WM_PLUGIN_NAME
					)
	...
	protected function create_index_file( $path ) {
		...
		if ( ! file_exists( $file ) ) {
			...
			if ( false === $handle ) {
				// We couldn't create a handle for the file, let's tell the user
				wp_die(
					__(
						'We were unable to create ' . $file . '. Try to change permissions of the parent folder or ' .
						'send us an email at support@servmask.com for assistance.',
						AI1WM_PLUGIN_NAME
					)
				);
			}
	...
	public function admin_head() {
		...
		<style type="text/css" media="all">
			...
			.toplevel_page_site-migration-export > div.wp-menu-image:before {
				...
				content: '';
				background: url(<?php echo $url; ?>/img/logo.svg) no-repeat center center;
		...
		wp_localize_script( 'ai1wm-js-import', 'ai1wm_import', array(
			...
			'oversize'   => sprintf(
				__(
					...
					'<a href="%s" target="_blank">Unlimited Extension</a>.',
					AI1WM_PLUGIN_NAME
				),
				size_format( apply_filters( 'ai1wm_max_file_size', AI1WM_MAX_FILE_SIZE ) ),
				AI1WM_UNLIMITED_EXT_URL
			),
			'invalid_extension' =>
				sprintf(
					__(
