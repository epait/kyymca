...
class Ai1wm_Extractor extends Ai1wm_Archiver {
	...
	public function extract_one_file_to( $location, $exclude = array() ) {
		...
		if ( in_array( $data['filename'], $exclude ) ) {
	...
	public function extract_by_files_array( $location, $files = array() ) {
		...
		while (
			...
			if ( $block === $this->eof ) {
				throw new Ai1wm_Not_Found_Exception(
					__( 'The following required files were not found in the archive: ', AI1WM_PLUGIN_NAME ) . implode( ', ', $files )
				);
			}
			...
			// do we have a match?
			if ( in_array( $data['filename'], $files ) ) {
	...
	private function set_file_mode( $file, $mode = 0755 ) {
