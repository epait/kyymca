...
abstract class Ai1wm_Import_Abstract {
	...
	public function start() {
		...
		Ai1wm_Status::set( array(
			...
			'processed' => 0,
			'type'      => 'info',
			'message'   => __( 'Unzipping archive...', AI1WM_PLUGIN_NAME ),
		) );

		// Open the archive file for reading
		$archive = new Ai1wm_Extractor( $this->storage()->archive() );

		// Unzip package.json and database.sql files
		...
		if ( $this->validate() ) {
			...
			throw new Ai1wm_Import_Exception(
				__(
					'Invalid archive file. It should contains <strong>package.json</strong> and ' .
					'<strong>database.sql</strong> as well.',
					AI1WM_PLUGIN_NAME
				)
			);
		}
		...
		Ai1wm_Status::set( array(
			...
			'message' => sprintf(
				__(
					...
					'<strong>1. You must save your permalinks structure twice. <a class="ai1wm-no-underline" href="%s" target="_blank">Permalinks Settings</a></strong> (opens a new window)<br />' .
	...
	protected function validate() {
		if ( is_file( $this->storage()->package() ) && is_file( $this->storage()->database() ) ) {
			return true;
		}
		return false;
	}
