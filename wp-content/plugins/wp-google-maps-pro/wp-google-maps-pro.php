...
function wpgmaps_tag_pro( $atts ) {
    ...
    if (isset($atts['lng']) && isset($atts['lat']) && isset($atts['parent_id'])) {
    ...
    if (isset($map_other_settings['list_markers_by']) && $map_other_settings['list_markers_by'] != "") {
        ...
        if ($map_other_settings['list_markers_by'] == "3") {
            if ($wpgmza_current_mashup) {
                $wpgmc = new wpgmza();
                $wpgmza_marker_list_output .= $wpgmc->list_markers(false,3,$wpgmza_mashup_parent_id,false,true,$wpgmza_mashup_ids);
            } else {
                $wpgmc = new wpgmza();
            ...
            if ($wpgmza_current_mashup) {
                $wpgmc = new wpgmza();
                $wpgmza_marker_list_output .= $wpgmc->list_markers(false,2,$wpgmza_mashup_parent_id,false,true,$wpgmza_mashup_ids);
            } else {
                $wpgmc = new wpgmza();
    ...
    if ($dbox_option == "5" || $dbox_option == "1" || !isset($dbox_option)) {
        ...
        $ret_msg = "
            ...
            <div style='clear:both;'>$wpgmza_marker_list_output</div>
        ...
        $ajax_nonce = wp_create_nonce("wpgmza");
            ...
            if (isset($wpgmza_short_code_array)) {
                foreach ($wpgmza_short_code_array as $wpgmza_cmd) {
                    ...
                    if ($wpgmza_current_map_shortcode_data[$wpgmza_current_map_id]['lat'] && $wpgmza_current_map_shortcode_data[$wpgmza_current_map_id]['lng'] && $wpgmza_current_map_shortcode_data[$wpgmza_current_map_id]['parent_id']) {
...
function wpgmaps_admin_javascript_pro() {
    ...
    var marker_pull = '<?php echo $marker_pull; ?>';
    var db_marker_array = JSON.stringify(<?php if (isset($markers)) { echo $markers; } else { echo ""; } ?>);
...
function wpgmza_datatables_update() {
    ...
    foreach ( $results as $result ) {
        ...
        if (!$category_icon) { if (!$icon) { $icon = $default_marker;  } else { $icon = "<img src='".$result->icon."' />"; } } 
        else { if (!$icon) { $icon = "<img src='".$category_icon."' />"; } else {  $icon = "<img src='".$result->icon."' />"; } }
        ...
        //var_dump($marker_category_data);
        if ($category_data == 'all') { $display++; } else {
            
            if (is_array($category_data)) { /*checkbox method */
                foreach ($category_data as $cat) {
                    foreach ($marker_category_data as $marker_cat) {
                        if ($marker_cat == $cat) { $display++; }
                    }
                }
            } else { /* select method */
                    foreach ($marker_category_data as $marker_cat) {
                        if ($marker_cat == $category_data) { $display++; }
                    }
            }
        }
        if ($display > 0 || $category_data == 'all') {
...
//    $wpgmza_tmp_head .= "<table id=\"wpgmza_table_".$map_id."\" class=\"wpgmza_table\" cellspacing=\"0\" cellpadding=\"0\" style=\"width:$width;\">";
    ...
    echo $wpgmza_tmp_head.$wpgmza_tmp_body.$wpgmza_tmp_footer;
    die();
...
function wpgmza_datatables_update_sl() {
    ...
    foreach ( $results as $result ) {
        ...
        $marker_category_data = explode(",",$marker_category);
        //var_dump($marker_category_data);
        if (is_array($marker_array)) { /*checkbox method */
...
if (isset($_GET['page']) && $_GET['page'] == 'wp-google-maps-menu-settings') {
...
function wpgmaps_sl_user_output_pro($map_id) {
    ...
    $ret_msg .= "       <input class=\"wpgmza_sl_reset_button\" type=\"button\" onclick=\"resetLocations($map_id)\" value=\"".__("Reset","wp-google-maps")."\"/>";
    $ret_msg .= "    </div>";
